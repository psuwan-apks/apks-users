...
$is_auth_page = ($currentPage === 'user' && in_array($currentAction, ['user-login', 'provider-login', 'user-register']))
             || ($currentPage === 'oauth' && in_array($currentAction, ['authorize', 'error']));
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>
        <?php 
        if ($is_auth_page) {
            if ($currentAction === 'user-register') {
                echo htmlspecialchars($translations['NAV_REGISTER_TOOLTIP'] ?? 'Register - APKS');
            } elseif ($currentAction === 'authorize') {
                echo 'Authorize - APKS';
            } elseif ($currentAction === 'error') {
                echo 'Error - APKS';
            } else {
                echo htmlspecialchars($translations['NAV_LOGIN_TOOLTIP'] ?? 'Sign In - APKS');
            }
        } else {
            echo htmlspecialchars($translations['APP_NAME'] ?? 'APKS');
        }
        ?>
    </title>
    ...
    <style>
        * {
        ...
        /* ─── Auth Page Fullscreen Centered Styling ─── */
        body.auth-page-body {
...
<body class="<?php echo $is_auth_page ? 'auth-page-body' : ''; ?>">
    ...
    <script>
        // Make SweetAlert2 globally accessible and auto-init Select2
        window.swal = Swal;
